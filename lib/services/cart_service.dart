  ...
  Future<Cart?> addToCart(SentCart sentCart, int userId) async {
    try {
      ...
      if (response.statusCode == 201) {
        ...
        print(
            "El usuario ya tiene un carrito activo o no se encontró: ${response.statusCode}");
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true &&
          ...
          return Cart.fromJson(
              data['response']['cart']); // Devuelve el carrito actualizado
        } else {
          print("El servidor no devolvió un carrito actualizado.");
          return null; // Maneja el caso en que no se devuelve el carrito
        }
      } else {
        print(
            "Error al actualizar el carrito. Código de estado: ${response.statusCode}");
        return null;
      }
    } catch (e) {
